const StoryCreatorModal = ({ theme, user, userProfile, onClose, setToast }: { theme: Theme, user: User, userProfile: any, onClose: () => void, setToast: (s: string) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <div className={`relative ${theme === 'dark' ? 'glass-dark border-white/10' : 'bg-white border-slate-200 shadow-2xl'} rounded-[32px] max-w-4xl w-full p-6 grid grid-cols-1 md:grid-cols-[1fr_300px] gap-6 animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh]`}>

        {/* Left Side: Portrait Interactive Live Preview */}
        <div
          ref={containerRef}
          className="relative aspect-[9/16] bg-zinc-950 rounded-2xl overflow-hidden border border-white/10 flex flex-col justify-center items-center shadow-inner max-h-[70vh] md:max-h-none mx-auto w-full max-w-[400px] touch-none select-none"
        ...
        >
          {(!imageUrl && !bgColor && showEntryMenu) ? (
            ...
            <img src={imageUrl} className="w-full h-full object-cover pointer-events-none" alt="preview" />
          ) : (
        ...
        <div className="space-y-6 flex flex-col justify-between">
          <div className="space-y-6">
            <div className="flex justify-between items-center pb-2 border-b border-zinc-200/50 dark:border-white/5">
              <h3 className={`text-base font-black uppercase tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-950'}`}>Create Story</h3>
              <button onClick={onClose} className={`p-1.5 rounded-full ${theme === 'dark' ? 'hover:bg-white/10 text-zinc-400' : 'hover:bg-slate-100 text-slate-800'}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={addText}
                className={`py-3 rounded-2xl text-[10px] font-black uppercase tracking-wider flex flex-col items-center justify-center gap-1 transition-all ${theme === 'dark' ? 'bg-zinc-800 hover:bg-zinc-700 text-white' : 'bg-slate-200 hover:bg-slate-300 text-slate-800'}`}
              >
                <span className="text-xl">📝</span>
                Add Text
              </button>
            ...
            {/* Element Editor removed, formatting moved to inline toolbars */}
